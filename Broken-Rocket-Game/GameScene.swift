...
// Floor height
let floor_distance:CGFloat = 10.0
...
extension Float {
    ...
    static func range(min: CGFloat, max: CGFloat) -> CGFloat {
        return CGFloat(random()) / 0xFFFFFFFF * (max - min) + min
    }
}
...
class GameScene: SKScene, SKPhysicsContactDelegate {
    ...
    func gameOver() {
        ...
        var currentStoredScore:Int = NSUserDefaults.standardUserDefaults().integerForKey("playerScore")
        if (currentStoredScore != 0 && currentStoredScore != nil) {
            if (currentStoredScore < score)
            {
                NSUserDefaults.standardUserDefaults().setInteger(score, forKey: "playerScore")
                currentStoredScore = score
            }
        } else {
            NSUserDefaults.standardUserDefaults().setInteger(score, forKey: "playerScore")
            currentStoredScore = score
        }
        var scoreString = "Top Score = " + String(currentStoredScore)
        var alertView:UIAlertView = UIAlertView(title: "Top Score", message: scoreString, delegate: self, cancelButtonTitle: "Ok")
        alertView.show()
    }
